...
class OrderServiceTests(TestCase):
    ...
    def test_server_recomputes_totals_and_ignores_hidden_values(self):
        ...
        self.assertRegex(order.order_number, r'^HF-\d{8}-\d{3}$')
...
class OrderViewTests(TestCase):
    ...
    def test_order_list_filters_by_exact_date(self):
        ...
        self.assertNotContains(response, 'HF-20260503-002')

    def test_order_list_print_uses_same_filters(self):
        self.client.force_login(self.user)
        matching = Order.objects.create(
            order_number='HF-20260503-003',
            customer=self.customer,
            date='2026-05-10',
            grand_total=Decimal('1200.00'),
        )
        Order.objects.create(
            order_number='HF-20260503-004',
            customer=self.customer,
            date='2026-05-11',
        ...
        self.assertNotContains(response, 'HF-20260503-004')

    def test_api_update_order_shortcut_status_success(self):
        self.client.force_login(self.user)
        order = Order.objects.create(
            order_number='HF-20260503-100',
            customer=self.customer,
            date='2026-05-03',
            status='pending',
        )
    ...
    def test_payment_report_view_filters_and_totals(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'HF-CASH-1')
        self.assertContains(response, 'HF-UPI-1')
